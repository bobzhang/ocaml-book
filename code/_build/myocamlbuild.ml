...
(** Ocaml pervasives really suck *)
let str_find src sub = 
  let len_sub = String.length sub 
  and len_src = String.length src 
  in 
  if len_sub > len_src then -1
  else (
    try(
      for i = 0 to len_src - len_sub do 
        try
          (
            for j = 0 to len_sub - 1 do 
              if src.[i+j] = sub.[j] then ()
              else raise (Str `NotFound)
            done ; 
            raise (Str (`Found i)))
        with 
          |Str `NotFound -> ()
...
let extensions () = 
  let pas = List.filter 
    (fun x -> str_find x "pa_" <> -1) (find_packages ()) in 
  let tbl = List.map 
    (fun pkg -> 
      let dir = 
