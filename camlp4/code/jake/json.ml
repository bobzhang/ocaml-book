...
(** some meta_basic is not what you want float, int, char, write one
    on your own let test _loc f = <:expr< $`flo:f$ >> it uses
    Camlp4_import.float_repres to translate a float it's different from
    string_of_float subtly
*)
type float' = float
...
module MetaExpr : sig
  val meta_t : Loc.t -> Jq_ast.t -> Ast.expr
end = struct 
...
module MetaPatt : sig
  val meta_t : Loc.t -> Jq_ast.t -> Ast.patt
end 
    = struct 
  let meta_float' _loc f =
    <:patt< $`flo:f$ >>
