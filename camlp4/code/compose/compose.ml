...
(** Look at ocamlbuild svn diff
    pp.ml defaults to camlp4o -printer o
*)
open Camlp4.PreCast;
...
(**
   other
   tuples (t1 * .. * tN)
   sum-types [ C1 of t1 | C2 of t2 ..| Cn of tn ]
   records {f1:t1; f2:t2; fn:tn}
   polymorphic variants [< `c1 of t1 | .. | `cN of tN ]

   notice they have different separator, and delimiter, you can build a incomplete
   ast when you have no delimiters
*)
